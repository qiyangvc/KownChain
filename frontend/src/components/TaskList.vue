...
function confirmAddTask() {
  ...
  emit('add-task', {
    ...
    tdStartTime: newTask.value.tdStartTime ? `${newTask.value.tdStartTime}:00` : '',
    tdEndTime: newTask.value.tdEndTime ? `${newTask.value.tdEndTime}:00` : '',
    tdDate: props.date.toISOString().slice(0, 10), // 统一使用UTC日期格式避免时区问题
