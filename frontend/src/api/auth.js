...
export default {
  ...
  // 新增待办
  async addTodo(data){
    console.log('添加TODO请求数据:', data);
    if (USE_MOCK_DATA) {
      return mockApi.addTodo(data);
    }
    try {
      const response = await apiClient.post('/todo/add', null, {
        params:{
          tdDate: data.tdDate,
          tdContent: data.tdContent,
          tdStartTime: data.tdStartTime,
          tdEndTime: data.tdEndTime,
          userID: data.userID
        }
      });
      console.log('添加TODO响应:', response);
      if (response.data && response.data.code !== 200) {
        console.error('新增待办失败，错误码:', response.data.code, '错误信息:', response.data.message);
        throw new Error(`新增待办失败：${response.data.message || '未知错误'}`);
      }
      return { data: response };
    } catch (error) {
      console.error('添加TODO时发生异常:', error);
      const errorMsg = error.response?.data?.message || error.message || '未知错误';
      throw new Error(`新增待办失败：${errorMsg}`);
    }
  },
  ...
  async deleteTodo(tdID) {
    if (USE_MOCK_DATA) {
      return mockApi.deleteTodo(tdID);
    }
    try {
      console.log('删除TODO请求参数:', tdID);
      const response = await apiClient.put('/todo/delete', null,{
        params:{
        tdid: tdID
      }
      });
  ...
  async modifyTodo(data) {
    console.log('修改TODO请求数据:', data);
    if (USE_MOCK_DATA) {
      return mockApi.modifyTodo(data);
    }
    try {
      const response = await apiClient.put('/todo/modify', null, {
        params:{
          tdID: data.tdID,
          tdDate: data.tdDate,
          tdContent: data.tdContent,
          tdStartTime: `${data.tdStartTime[0]}${data.tdStartTime[1]}:${data.tdStartTime[3]}${data.tdStartTime[4]}:00`,
          tdEndTime: `${data.tdEndTime[0]}${data.tdEndTime[1]}:${data.tdEndTime[3]}${data.tdEndTime[4]}:00`
        }
      });
      console.log('修改TODO响应:', response);
      return { data: response };
    } catch (error) {
      throw error;
    }
  ...
  // 按用户ID和日期获取待办列表
  async getTodoByUidAndDate(params) {
    console.log('获取TODO列表请求参数:', params);
    if (USE_MOCK_DATA) {
      return mockApi.getTodoByUidAndDate(params);
    }
    try {
      const response = await apiClient.get('/todo/getByUidAndDate', {
        params:{
          userID: params.userID,
          tdDate: params.tdDate
        }
      });
      console.log('获取TODO列表响应:', response);
      return { data: response };
    } catch (error) {
      throw error;
    }
